...
There are two implementations of this plugin, Perlbal::Plugin::MogileFS which is the synchronous implementation that stalls the Perlbal event loop, so you need fast trackers to keep up with demand. The second implementation Perlbal::Plugin::AsyncMogileFS is asynchronous and does not stall the event loop waiting for slow trackers to respond. It is suggested you use the latter in production.
